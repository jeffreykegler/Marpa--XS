...
L<Marpa::HTML> recognizes elements even if they have missing
start and/or end tags.
L<Marpa::HTML> can B<supply missing tags>:
...
perltidy: '-dcsc -sil=0'
    ...
    sub supply_missing_tags {
        ...
        return ( Marpa::HTML::start_tag() // "<$tagname>\n" )
            . Marpa::HTML::contents() .
...
L<Marpa::HTML> understands the hierarchical structure of an HTML document.
...
L<Marpa::HTML> is an extremely liberal HTML parser.
L<Marpa::HTML> does not reject any documents,
no mater how poorly they fit the HTML standards.

=head1 THE Marpa::HTML::html STATIC METHOD

The interface to 
L<Marpa::HTML> is through the
C<Marpa::HTML::html>
static method.
It is
the only
...
Handler descriptions in L<Marpa::HTML> are key-value pairs
in a hash.
...
L<Marpa::HTML> are case-sensitive, and must be all uppercase.
...
become the terminals for L<Marpa::HTML>.
...
L<Marpa::HTML> reclasses as cruft.

L<Marpa::HTML> reclasses terminals as cruft when
they do not fit the structure of an HTML document.
One example of a terminal that
L<Marpa::HTML> would reclass as cruft is a
...
L<Marpa::HTML> leaves interpretation of markup and entities entirely to
the application.
...
Cruft can also be found here, though for L<Marpa::HTML> that is a
last resort.

=head2 :WHITESPACE

A L<Marpa::HTML> C<:WHITESPACE> terminal is created for every
...
that the action semantics of L<Marpa::HTML> are natural.
...
through the L</"Marpa::HTML::original"> API method.
...
L<HTML::PullParser> does the job with significantly lower overhead.
...
The original view is seen
through the L</"Marpa::HTML::values"> API method.
...
The descendants view is implemented via the L</"Marpa::HTML::descendants"> method.
...
In terms of L<Marpa::HTML> views,
...
For L<Marpa::HTML> this is close to the worst case.

L<Marpa::HTML> optimizes for unvalued components.
...
L<Marpa::HTML>'s semantic action API is implemented
...
=item Marpa::HTML::values

The C<Marpa::HTML::values> method implements the values view.
...
The C<Marpa::HTML::descendants> method takes a dataspec
as its argument.
The C<Marpa::HTML::descendants> method returns a reference
...
If the descendant is a pseudoclass component,
returns the
...
Each L<Marpa::HTML> instance
makes available
a per-instance variable
as a scratchpad for the application:
C<$Marpa::HTML::INSTANCE>.
Each call to
L<Marpa::HTML::html|/"THE Marpa::HTML::html STATIC METHOD">
creates a C<$Marpa::HTML::INSTANCE>
variable which is
...
C<$Marpa::HTML::INSTANCE> can be used to
store the data.

Ordinarily, C<$Marpa::HTML::INSTANCE> is destroyed,
with the rest of the parse instance,
when C<Marpa::HTML::html> returns.
But it can be useful
for the C<:TOP> semantic action to
return
C<$Marpa::HTML::INSTANCE> as the value of the parse.
...
under which
L<Marpa::HTML> will recognize a virtual end tag.
At end of file,
as one example,
L<Marpa::HTML> will do its best to produce a balanced
HTML structure by
...
L<Marpa::HTML> exports nothing by default.
Optionally,
L<Marpa::HTML::html|/"THE Marpa::HTML::html STATIC METHOD">
may be exported.
